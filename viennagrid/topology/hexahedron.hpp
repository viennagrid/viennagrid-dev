  ...
  namespace topology
  {
    ...
    struct bndcell_filler<hexahedron_tag, 1>
    {
      ...
      static void fill(ElementContainerType & elements, const VertexContainerType & vertices, OrientatationContainerType & orientations, Segment & seg)
      {
        ...
        edgevertices[1] = vertices[7];
        edge.vertices(edgevertices);
        elements[11] = seg.push_back(edge, &orientations[1]);
    }
      ...
      static void fill(ElementContainerType & elements, const VertexContainerType & vertices, Segment & seg)
      {
        ...
        edgevertices[1] = vertices[5];
        edge.vertices(edgevertices);
        elements[6] = seg.push_back(edge, NULL);

        edgevertices[0] = vertices[3];
        edgevertices[1] = vertices[7];
        edge.vertices(edgevertices);
        elements[7] = seg.push_back(edge, NULL);
        
        edgevertices[0] = vertices[4];
        edgevertices[1] = vertices[5];
        edge.vertices(edgevertices);
        elements[8] = seg.push_back(edge, NULL);

        edgevertices[0] = vertices[4];
        edgevertices[1] = vertices[6];
        edge.vertices(edgevertices);
        elements[9] = seg.push_back(edge, NULL);
        
        edgevertices[0] = vertices[5];
        edgevertices[1] = vertices[7];
        edge.vertices(edgevertices);
        elements[10] = seg.push_back(edge, NULL);
        
        edgevertices[0] = vertices[6];
        edgevertices[1] = vertices[7];
        edge.vertices(edgevertices);
        elements[11] = seg.push_back(edge, NULL);
    }
    ...
    struct bndcell_filler<hexahedron_tag, 2>
    {
      ...
      static void fill(ElementContainerType & elements, const VertexContainerType & vertices, OrientatationContainerType & orientations, Segment & seg)
      {
        typename VertexContainerType::value_type facetvertices[4];
      ...
      static void fill(ElementContainerType & elements, const VertexContainerType & vertices, Segment & seg)
      {
        ...
        facetvertices[3] = vertices[7];
        facet.vertices(facetvertices);
        elements[4] = seg.push_back(facet, NULL);
        elements[4]->fill(seg);
        ...
        facetvertices[3] = vertices[7];
        facet.vertices(facetvertices);
        elements[5] = seg.push_back(facet, NULL);
        elements[5]->fill(seg);
    }
    };
