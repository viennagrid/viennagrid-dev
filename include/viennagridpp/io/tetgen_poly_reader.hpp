...
namespace viennagrid
{
  namespace io
  {
    ...
    struct tetgen_poly_reader
    {
       ...
       */
      template <typename MeshT>
      void operator()(MeshT const & mesh_obj, std::string const & filename,
                     std::vector< typename viennagrid::result_of::point<MeshT>::type > & hole_points,
                     std::vector< std::pair<typename viennagrid::result_of::point<MeshT>::type, int> > & seed_points) const
      {
...
//           throw bad_file_format_exception("* ViennaGrid: tetgen_poly_reader::operator(): File " + filename + ": POLY point dimension missmatch");
        ...
        std::map<VertexIDType, VertexType> vertex_map;
        ...
        for (int i=0; i<node_num; i++)
        {
          ...
          vertex_map[id] = viennagrid::make_vertex( mesh_obj, p );
        }
        ...
        for (int i=0; i<facet_num; ++i)
        {
          ...
          std::list<LineType> lines;
          std::list<VertexType> vertices;
          ...
          for (int j = 0; j<polygon_num; ++j)
          {
            ...
            std::vector<VertexType> vertex_handles(static_cast<std::size_t>(vertex_num));
            ...
            for (std::size_t k = 0; k<static_cast<std::size_t>(vertex_num); ++k)
            {
              ...
              typename std::map<VertexIDType, VertexType>::iterator it = vertex_map.find( id );
              if (it == vertex_map.end())
                throw bad_file_format_exception("* ViennaGrid: tetgen_poly_reader::operator(): Vertex ID not found");

              vertex_handles[k] = vertex_map[id];
            }
...
//               vertices.push_back( vertex_handles.front() );
              lines.push_back( viennagrid::make_line(mesh_obj, vertex_handles.front(), vertex_handles.front()) );
            }
            ...
            {
              typename std::vector<VertexType>::iterator it1 = vertex_handles.begin();
              typename std::vector<VertexType>::iterator it2 = it1; ++it2;
          ...
          PLCType plc = viennagrid::make_element<viennagrid::plc_tag>( mesh_obj, lines.begin(), lines.end() );
          for (typename std::list<PointType>::const_iterator hpit = hole_points_read.begin(); hpit != hole_points_read.end(); ++hpit)
