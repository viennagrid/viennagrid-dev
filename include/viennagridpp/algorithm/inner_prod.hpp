...
//     };
    ...
    typename viennagrid::result_of::coord<PointT1>::type inner_prod_impl(PointT1 const & p1, PointT2 const & p2)
    {
      assert( p1.size() == p2.size() );
      typename viennagrid::result_of::coord<PointT1>::type result(0);
      for (typename PointT1::size_type i = 0; i != p1.size(); ++i)
        result += p1[i]*p2[i];
      return result;
    }
  }
...
//   }
  ...
  {
    return detail::inner_prod_impl(p1, p2);
