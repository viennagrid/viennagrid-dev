...
namespace viennagrid
{
  ...
  void edge_refine(base_mesh<mesh_is_const> const & mesh_in,
                   viennagrid_bool * edge_refinements,
                   viennagrid_numeric * vertex_locations,
                   mesh const & mesh_out)
  {
    THROW_ON_ERROR( viennagrid_mesh_refine_edges(mesh_in.internal(), edge_refinements, vertex_locations, mesh_out.internal()) );
  }
  ...
  void edge_refine(base_mesh<mesh_is_const> const & mesh_in,
                   ...
                   mesh const & mesh_out)
  {
    THROW_ON_ERROR( viennagrid_mesh_refine_edges(mesh_in.internal(), edge_refinements, NULL, mesh_out.internal()) );
  }
  ...
  void ensure_longest_edge_refinement(base_mesh<mesh_is_const> const & mesh_in,
                                      viennagrid_bool * edge_refinements)
  {
    THROW_ON_ERROR( viennagrid_mesh_refine_ensure_longest_edge(mesh_in.internal(), edge_refinements) );
  }
  ...
  void element_refine(base_mesh<mesh_is_const> const & mesh_in,
                      ...
                      mesh const & mesh_out)
  {
    std::vector<viennagrid_bool> flags( viennagrid::elements(mesh_in,1).size() );
    THROW_ON_ERROR( viennagrid_mesh_refine_create_uniformly_edge_flags(mesh_in.internal(), elements_to_refine_count, elements_to_refine, &flags[0]) );

    edge_refine(mesh_in, &flags[0], mesh_out);
  }
                             ...
                             mesh const & mesh_out)
  {
    ...
    THROW_ON_ERROR( viennagrid_mesh_elements_get(mesh_in.internal(), viennagrid::cell_dimension(mesh_in), &cells_begin, &cells_end) );

    element_refine(mesh_in, cells_end-cells_begin, cells_begin, mesh_out);
  }
  ...
  template<bool mesh_is_const>
  void hyperplane_refine(viennagrid::base_mesh<mesh_is_const> const & src_mesh,
                         viennagrid::point const & hyperplane_point,
                         viennagrid::point const & hyperplane_normal,
                         viennagrid_numeric tolerance,
                         viennagrid::mesh const & mesh_out)
  {

    std::vector<viennagrid_bool> edge_refine_flags( viennagrid::elements(src_mesh, 1).size() );
    std::vector<viennagrid_numeric> vertex_locations( viennagrid::elements(src_mesh, 1).size()*viennagrid::geometric_dimension(src_mesh) );

    THROW_ON_ERROR( viennagrid_mesh_refine_create_hyperplane_edge_flags(src_mesh.internal(),
                                                                        const_cast<viennagrid_numeric*>(&hyperplane_point[0]), const_cast<viennagrid_numeric*>(&hyperplane_normal[0]),
                                                                        &edge_refine_flags[0], &vertex_locations[0],
                                                                        tolerance) );

    edge_refine(src_mesh, &edge_refine_flags[0], &vertex_locations[0], mesh_out);
  }
...
//                    const viennagrid_bool * cell_refinements,
//                    mesh & mesh_out)
//   {
//     std::vector<viennagrid_bool>
//
//
// //     THROW_ON_ERROR( viennagrid_mesh_edge_refine(mesh_in.internal(), edge_refinements, NULL, mesh_out.internal()) );
//   }
