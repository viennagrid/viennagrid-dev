...
namespace viennagrid
{
  namespace detail
  {
    ...
    inline viennagrid::result_of::coord<point_t>::type
    distance_impl(point_t const & p1,
                  point_t const & p2)
    {
      return norm_2(p1 - p2);
    }
    ...
    inline viennagrid::result_of::coord<point_t>::type
    distance_impl( std::pair<point_t,point_t> const & pts)
    {
    ...
    viennagrid::result_of::coord<point_t>::type
...
//     }
                  ...
                  base_element<element2_is_const> const & el2)
    {
      if (el1.tag().is_vertex() && el2.tag().is_vertex())
        return distance_impl(accessor(el1), accessor(el2));
      if (el1.tag().is_vertex())
        return distance_impl(accessor, accessor(el1), el2);
...
//     }
    ...
    inline viennagrid::result_of::coord<point_t>::type
    boundary_distance_impl(point_t const & p1,
                           point_t const & p2)
    {
      return norm_2(p1 - p2);
    }
    ...
    inline viennagrid::result_of::coord<point_t>::type
    boundary_distance_impl( std::pair<point_t,point_t> const & pts)
    {
      return boundary_distance_impl(pts.first, pts.second);
    }

    template <typename PointAccessorT>
    typename viennagrid::result_of::coord<point_t>::type
...
//     }
    ...
    {
      if (el1.tag().is_vertex() && el2.tag().is_vertex())
        return boundary_distance_impl(accessor, accessor(el1), accessor(el2));
      if (el1.tag().is_vertex())
        return boundary_distance_impl(accessor, accessor(el1), el2);
      if (el2.tag().is_vertex())
        return boundary_distance_impl(accessor, el1, accessor(el2));
  ...
  typename result_of::coord< base_mesh_region<mesh_region_is_const> >::type distance( base_mesh_region<mesh_region_is_const> const & region, base_element<element_is_const> const & line )
  {
  ...
  boundary_distance(SomethingT1 const & el1,
                    SomethingT2 const & el2)
  {
    return detail::boundary_distance_impl( point_accessor(el1), el1, el2 );
  }
  ...
  template <typename SomethingT>
  typename viennagrid::result_of::coord<SomethingT>::type
  boundary_distance(point_t const & el1,
                    SomethingT const & el2)
  {
    return detail::boundary_distance_impl( point_accessor(el2), el1, el2 );
  }
