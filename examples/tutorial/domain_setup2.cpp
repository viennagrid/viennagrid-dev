...
int main()
{
  ...
  domain.push_back(cell); //copies 'cell' to the domain. 'cell' can be reused for setting up the other cells.
  ...
  domain.push_back(cell); // Note that we push to 'seg1' now.
  ...
  std::cout << "Cells in domain:" << std::endl;
  CellRange cells_seg0 = viennagrid::ncells(domain);
