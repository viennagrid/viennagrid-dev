...
// Only valid for meshs using the configuration class viennagrid::config::tetrahedral_3d
//
...
//
// Disable edges for all triangles for all possible mesh types
...
//
// Disable triangular facets of a tetrahedron for all possible mesh types
//
...
int main()
{
  ...
  typedef viennagrid::result_of::ncell_range<Mesh, 0>::type       VertexRange;
  ...
  std::cout << "Total memory required: "
            << (viennagrid::ncells<0>(mesh).size() * sizeof(VertexType)) / 1024.0
